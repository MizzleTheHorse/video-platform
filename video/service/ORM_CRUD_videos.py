...
class DatabaseInterface:
        ...
        with Session() as session:
            try:
                if id:
                    ...
                    if result:
                        video = Video(video_id=result.video_id, user_id=result.user_id, title=result.title, resume=result.resume, category_id=result.category_id)
                        return video
                    else: 
                        return 'not found'
                else:
    ...
    def post_video(self, user_id, title, resume=None, category_id=None):
        with Session() as session:
            try: 
